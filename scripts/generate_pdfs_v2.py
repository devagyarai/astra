import os
import subprocess

EDGE_EXE = r"C:\Program Files (x86)\Microsoft\Edge\Application\msedge.exe"
if not os.path.exists(EDGE_EXE):
    EDGE_EXE = r"C:\Program Files\Google\Chrome\Application\chrome.exe"

BASE_DIR = r"c:\Development\astra\brand_assets\v2"

def export_pdf(svg_p, pdf_p):
    html_p = pdf_p + ".html"
    svg_uri = "file:///" + os.path.abspath(svg_p).replace("\\", "/")
    html_content = f"""<!DOCTYPE html>
<html>
<head>
<style>
    @page {{ size: auto; margin: 0; }}
    html, body {{ margin: 0; padding: 0; background: #070A14; width: 100%; height: 100%; overflow: hidden; }}
    img {{ width: 100%; height: 100%; object-fit: contain; display: block; }}
</style>
</head>
<body>
    <img src="{svg_uri}" />
</body>
</html>"""
    with open(html_p, "w", encoding="utf-8") as f:
        f.write(html_content)
    
    cmd = [
        EDGE_EXE,
        "--headless",
        "--disable-gpu",
        "--no-pdf-header-footer",
        f"--print-to-pdf={pdf_p}",
        f"file:///{html_p.replace('\\', '/')}"
    ]
    subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    if os.path.exists(html_p):
        os.remove(html_p)
    print(f"Generated v2 PDF vector container: {pdf_p}")

svg_board = os.path.join(BASE_DIR, "guidelines", "astra-brand-guidelines-v2-board.svg")
pdf_board = os.path.join(BASE_DIR, "guidelines", "astra-brand-guidelines-v2-board.pdf")
svg_logo = os.path.join(BASE_DIR, "vectors", "astra-logo-primary-dark-v2.svg")
pdf_logo = os.path.join(BASE_DIR, "raster", "astra-logo-primary-dark-v2.pdf")

export_pdf(svg_board, pdf_board)
export_pdf(svg_logo, pdf_logo)

# Clean temp htmls
for root, dirs, files in os.walk(BASE_DIR):
    for f in files:
        if f.endswith(".html"):
            try:
                os.remove(os.path.join(root, f))
            except:
                pass

print("v2 PDF Export Complete.")
