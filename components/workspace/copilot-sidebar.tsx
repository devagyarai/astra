...
import { BiasVisualization, TradeoffVisualization } from "@/components/ai/visualizations";
import { BiasAnalysis, TradeoffMatrix, AiInsights } from "@/types/analysis";
import { analyzeDecision, DecisionAnalysis } from "@/lib/analysis/decision-analysis";
import { HybridAnalysis } from "@/components/ai/hybrid-analysis";
...
export function CopilotSidebar({ decision, updateField, isMobile = false }: CopilotSidebarProps) {
  ...
  const [coreAnalysis, setCoreAnalysis] = useState<DecisionAnalysis | null>(null);
  ...
  const handleAnalyzeHybrid = async () => {
    setIsAnalyzing(true);
    
    // 1. Run Astra Core Deterministic Evaluation locally
    const deterministic = analyzeDecision(decision);
    setCoreAnalysis(deterministic);
    
    // 2. Check if AI provider is configured and fetch AI enhancement
    if (apiKeyExists) {
      try {
        ...
        console.error("Hybrid AI Analysis Error:", e);
      }
    }
      ...
      fetch: async (url, options) => {
        ...
        return fetch(url, {
          ...
          body: JSON.stringify({
            ...parsedBody,
            provider: aiSettings.provider,
            system: "You are an expert Decision Intelligence Partner (Astra). Answer concisely, use markdown, tables, and lists. Focus on helping the user make the best possible decision.",
  ...
  return (
    <aside className={isMobile ? "flex flex-col h-full w-full" : "flex flex-col h-full w-full border-l border-border/50 bg-background/50"}>
      {/* Header & Tabs */}
      <div className="p-4 border-b border-border/50 bg-card/40">
        <div className="flex items-center gap-2 mb-4">
          <LineChart className="h-4 w-4 text-primary" />
          <h3 className="font-semibold text-sm tracking-tight uppercase">Decision Intelligence</h3>
        </div>
        <div className="flex gap-1 bg-secondary/50 p-1 rounded-lg">
          <button 
            onClick={() => setActiveTab("analysis")}
            className={`flex-1 text-xs font-medium py-1.5 rounded-md transition-all ${activeTab === "analysis" ? "bg-background shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground"}`}
          >
            Analysis
          </button>
          <button 
          ...
          {activeTab === "analysis" && (
            ...
            >
              <ScrollArea className="flex-1 p-6">
                {!coreAnalysis && !isAnalyzing ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    <div className="h-12 w-12 rounded-full bg-primary/10 flex items-center justify-center mb-4">
                      <Zap className="h-6 w-6 text-primary" />
                    </div>
                    <h4 className="font-semibold text-base mb-2">Analyze Decision</h4>
                    <p className="text-sm text-muted-foreground max-w-[240px] mb-6">
                      Evaluate your goal, options, and constraints to generate structured insights.
                    </p>
                    <Button onClick={handleAnalyzeHybrid} className="w-full sm:w-auto px-8 shadow-sm" size="lg">
                      Run Analysis
                    </Button>
                  </div>
                ) : (
                  <div className="space-y-6 pb-6">
                    <div className="flex justify-end">
                       <Button onClick={handleAnalyzeHybrid} variant="outline" size="sm" disabled={isAnalyzing}>
            ...
            >
              {!apiKeyExists ? (
                <div className="flex flex-col items-center justify-center h-full text-center p-8">
                  <div className="h-12 w-12 rounded-full bg-secondary flex items-center justify-center mb-4 shadow-sm">
                    <MessageSquare className="h-5 w-5 text-muted-foreground opacity-50" />
                  </div>
                  <h4 className="font-semibold text-sm mb-1 tracking-tight">Chat Offline</h4>
                  <p className="text-xs text-muted-foreground max-w-[200px] leading-relaxed mb-4">
                    Connect OpenAI or Anthropic in Settings to enable the AI Chat partner.
                  </p>
                  <a href="/settings" className="inline-flex items-center justify-center rounded-md text-xs font-medium bg-primary text-primary-foreground h-8 px-4 shadow">
                    Settings
                  </a>
                </div>
              ) : (
                  ...
                  <ScrollArea className="flex-1 p-4" ref={scrollRef}>
                    {messages.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-[300px] text-center opacity-70 p-6">
                        ...
                        <h4 className="font-semibold text-sm mb-1 tracking-tight">AI Partner Ready</h4>
                        <p className="text-xs text-muted-foreground max-w-[200px] leading-relaxed">
                          Ask a question or select a tool to start improving your decision.
                        </p>
                      </div>
                    ) : (
            ...
            >
              {!apiKeyExists ? (
                <div className="flex flex-col items-center justify-center h-full text-center p-8">
                  ...
                  <h4 className="font-semibold text-sm mb-1 tracking-tight">Tools Offline</h4>
                  <p className="text-xs text-muted-foreground max-w-[200px] leading-relaxed mb-4">
                    Connect an AI provider to unlock Quick Actions and Visualizations.
                  </p>
                  <a href="/settings" className="inline-flex items-center justify-center rounded-md text-xs font-medium bg-primary text-primary-foreground h-8 px-4 shadow">
                    Settings
                  </a>
                ...
                <>
                  ...
                  {actionResult && activeAction === "goal" ? (
                  ...
                  ) : actionResult && (activeAction === "bias" || activeAction === "tradeoff") ? (
                    <div className="space-y-4">
                      <div className="flex justify-between items-center">
                        <h4 className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Result</h4>
                        <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => { setActionResult(null); setActiveAction(null); }}>
                          Back to Tools
                        </Button>
                      </div>
                    ...
                    <>
                      <ActionCard 
                        ...
                        isAnalyzing={isAnalyzing && activeAction === "goal"}
                      ...
                      <ActionCard 
                        ...
                        isAnalyzing={isAnalyzing && activeAction === "bias"}
                        activeAction={activeAction}
                        actionKey="bias"
                      ...
                      <ActionCard 
                        ...
                        isAnalyzing={isAnalyzing && activeAction === "tradeoff"}
                        activeAction={activeAction}
                        actionKey="tradeoff"
